...
echo "        (make note of the GPG key ID to be used)"
echo "    gopass"
echo "        (this will throw an error that gopass isn't initialized, but that's OK."
echo "        We just want gopass to create its default directories.)"
echo "    cd ~/.local/share/gopass/stores/root"
echo "    git init"
echo "    git config user.signingkey <gpg-key-ID>"
echo "    git config user.email \"2258100-gkotian@users.noreply.gitlab.com\""
echo "    gopass init <gpg-key-ID>"
echo "        (when it asks for an email address, use 2258100-gkotian@users.noreply.gitlab.com)"
